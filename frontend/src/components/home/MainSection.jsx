      ...
      <AnimatePresence mode="wait">
        {isFadingOut ? (
          ...
          >
            ...
            <motion.button
              className="px-6 py-3 font-semibold tracking-wider bg-[#C0C0C0] text-[#090a29] rounded-full transition hover:bg-gray-300 flex items-center justify-center"
              onMouseEnter={() => setIsHovered(true)} // Set hover state
              onMouseLeave={() => setIsHovered(false)} // Reset hover state
              transition={{ delay: 0.7 }} // Delay for button
            >
              Enquire Now
              {isHovered && <MdArrowForward className="ml-2 transition-transform transform" />} {/* Conditionally render the icon */}
            </motion.button>
          </motion.div>
        ) : (
          <motion.div
